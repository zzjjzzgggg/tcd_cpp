...
void sampleGraph(const Graph& G) {
    ...
    for (int r = 0; r < FLAGS_r; ++r) {
        ...
        for (auto ni = G.beginNI(); ni != G.endNI(); ++ni) {
            if (rng.uniform() > FLAGS_p) continue;
            auto&& nd = ni->second;
            std::vector<int> nodes;
            nodes.push_back(ni->first);
            auto nbrs = nd.getNbrs();
            nodes.insert(nodes.end(), nbrs.begin(), nbrs.end());
            auto sub = graph::getSubgraph(G, nodes);
            for (auto&& ei = sub.beginEI(); ei != sub.endEI(); ++ei) {
                int src = ei.getSrcID(), dst = ei.getDstID();
                if (!S.isEdge(src, dst)) S.addEdge(src, dst);
            }
        }
